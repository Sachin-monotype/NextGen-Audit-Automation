    ...
    *,
    ...
    if not isinstance(spec, dict):
        return [
            ScopeCheck(
                operation=operation,
                field_path="enrichmentScope",
                match_status="SKIP",
                notes="Operation not in enrichment_scope_manifest.json",
            )
        ]
    ...
    if gap:
        rows.append(
            ScopeCheck(
                ...
                match_status="SKIP",
                notes=(
                    f"Produce≠require gap: implementation={impl.get('scope')} "
                    f"enforced={enf.get('scope')} (informational)"
                ),
